import pytest
import numpy as np
from CalSciPy.trace_processing import calculate_dfof


def test_calculate_dfof(sample_traces, extended_sample_traces,
                        dfof_sample_traces, dfof_offset_sample_traces,
                        dfof_ext_sample_traces, dfof_frame_rate_sample_traces):
    # standard
    dfof = calculate_dfof(sample_traces)
    np.testing.assert_allclose(dfof, dfof_sample_traces, rtol=1e-3,
                               err_msg="Failed calculating standard dfof")
