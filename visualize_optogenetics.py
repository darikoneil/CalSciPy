...
def view_target_overlay(photostimulation: Photostimulation, targets=None):
    ...
    with plt.style.context("CalSciPy.main"):
        ...
        if targets is None:
            ...
            colors = _generate_colormap_spectrum(len(photostimulation.stim_sequence), colormap="magma", alpha=.75)

            for idx, group in enumerate(photostimulation.stim_sequence):
...
def view_targets(photostimulation: Photostimulation, targets=None):
    ...
    with plt.style.context("CalSciPy.main"):
        ...
        if targets is None:
            ...
            colors = _generate_colormap_spectrum(len(photostimulation.stim_sequence), alpha=0.75)
...
def _view_spiral_masked_targets(photostimulation: Photostimulation, targets=None):
    ...
    with plt.style.context("CalSciPy.main"):
        ...
        if targets is None:
            ...
            for group in photostimulation.stim_sequence:
