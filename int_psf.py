...
#### create context
dpg.create_context()
dpg.create_viewport(title='CalSciPy - Point Spread Function', width=720, height=640)

images = psf.denoised[25, :, :]
mi = np.min(images)
ma = np.max(images)
with dpg.window(label="Main_Window", width=-1, height=-1):
    with dpg.group(horizontal=True):
        dpg.add_colormap_scale(min_scale=int(mi), max_scale=int(ma), height=400)
        with dpg.plot(label="PSF", no_mouse_pos=True, height=400, width=-1):
            ...
            with dpg.plot_axis(dpg.mvYAxis, label="y", no_gridlines=True, no_tick_marks=True, lock_min=True, lock_max=True):
                dpg.add_heat_series(images, 256, 256, scale_min=int(mi), scale_max=int(ma))
